...
function normalizeDistricts(rows){if(!rows.length)return fallbackDistricts.map(x=>({name:x[0],alias:x[1],priority:x[2],pads:x[3],people:x[4],active:true}));return rows.map(r=>({name:r.name,alias:r.city_alias||'',priority:String(r.priority).toUpperCase()==='TRUE',pads:Number(r.pads_distributed||0),people:Number(r.people_helped||0),active:String(r.active).toUpperCase()!=='FALSE'})).filter(x=>x.active)}
let districts=[];
function focusCard(d){const bg=focusImages[d.name]||focusImages.Ranchi;return `<article class="focus-card"><div class="focus-photo" style="background-image:url('${bg}')"><strong>${d.name}${d.alias?`<span>(${d.alias})</span>`:''}</strong></div><div class="focus-metrics"><div class="metric-row"><div class="metric-icon">♧</div><div><b>${d.pads.toLocaleString('en-IN')}</b><small>Pads distributed</small></div></div><div class="metric-row"><div class="metric-icon">♙</div><div><b>${d.people.toLocaleString('en-IN')}</b><small>People helped</small></div></div></div></article>`}
function miniCard(d){return `<article class="mini"><strong>${d.name}${d.alias?` (${d.alias})`:''}</strong><div class="mini-metrics"><span><b>${d.pads.toLocaleString('en-IN')}</b> pads distributed</span><span><b>${d.people.toLocaleString('en-IN')}</b> people helped</span></div></article>`}
